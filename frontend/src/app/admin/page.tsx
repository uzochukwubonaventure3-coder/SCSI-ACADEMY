...
import {
  LayoutDashboard, FileText, Video, Users, Mail, MessageSquare, User,
...
function BlogManager({token,toast}:{token:string;toast:ReturnType<typeof useToast>}) {
  ...
  const [form,setForm]     = useState({
    title:'',slug:'',excerpt:'',content:'',cover_image:'',tags:'',status:'draft',
    isFree:false,previewUrl:'',previewEndSeconds:60,discountPercent:0,discountExpiresAt:'',outcomes:'',targetAudience:'',lessons:''
  })
  ...
  const save = async()=>{
    ...
    try{
      ...
      setForm({title:'',slug:'',excerpt:'',content:'',cover_image:'',tags:'',status:'draft',isFree:false,previewUrl:'',previewEndSeconds:60,discountPercent:0,discountExpiresAt:'',outcomes:'',targetAudience:'',lessons:''})
  ...
  const editPost = (p:BlogPost)=>{setForm({title:p.title,slug:p.slug,excerpt:p.excerpt||'',content:p.content||'',cover_image:p.cover_image||'',tags:(p.tags||[]).join(', '),status:p.status,isFree:false,previewUrl:'',previewEndSeconds:60,discountPercent:0,discountExpiresAt:'',outcomes:'',targetAudience:'',lessons:''});setEditing(p.id);setShowForm(true)}

  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
        <h2 className="h-serif" style={{fontSize:'1.25rem',fontWeight:700}}>Blog Posts</h2>
        <button onClick={()=>{setShowForm(!showForm);setEditing(null);setForm({title:'',slug:'',excerpt:'',content:'',cover_image:'',tags:'',status:'draft',isFree:false,previewUrl:'',previewEndSeconds:60,discountPercent:0,discountExpiresAt:'',outcomes:'',targetAudience:'',lessons:''})}}
...
function VideoManager({token,toast}:{token:string;toast:ReturnType<typeof useToast>}) {
  ...
  const [form,setForm]     = useState({
    title:'',slug:'',description:'',video_url:'',duration:'',tags:'',status:'draft',
    priceKobo:200000,isFree:false,category:'',previewUrl:'',previewEndSeconds:60,discountPercent:0,discountExpiresAt:'',outcomes:'',targetAudience:'',lessons:''
  })
  const [editing,setEditing] = useState<number|null>(null)
  const [showForm,setShowForm] = useState(false)
  const [saving,setSaving] = useState(false)
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',background:'var(--bg-0)',fontFamily:'var(--font-sans)'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'auto'}}>
        ...
        {/* Content */}
        <main style={{flex:1,overflowY:'auto',padding:'clamp(1rem,3vw,1.75rem) clamp(1rem,3vw,2rem)',maxWidth:'1100px',margin:'0 auto',width:'100%',boxSizing:'border-box'}}>
          {impersonating
            ? null
            : <>
                {tab==='dashboard'&&(
