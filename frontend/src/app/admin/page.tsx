...
import {
  ...
  Send, BookOpen, UserCheck, Eye, Activity, Search,
...
type Tab = 'dashboard'|'blog'|'videos'|'gallery'|'students'|'contacts'|'registrations'|'testimonials'|'subscribers'|'broadcast'|'settings'
        ...
        <div style={{background:'var(--bg-1)',border:'1px solid var(--border)',borderRadius:'var(--radius-xl)',padding:'1.75rem',display:'flex',flexDirection:'column',gap:'1.125rem'}}>
          ...
          <div className="fgroup">
            ...
            <select value={form.audience} className="fi" onChange={e=>setForm(p=>({...p,audience:e.target.value}))}>
              ...
              <option value="inactive">Inactive / Incomplete Registrations</option>
              <option value="newsletter">Newsletter Subscribers Only</option>
            </select>
            <p style={{marginTop:'0.375rem',fontSize:'0.72rem',color:'var(--txt-3)'}}>
              {form.audience==='inactive'?'Sends to users who registered but did not complete payment or are no longer active.':form.audience==='newsletter'?'Sends to weekly audit newsletter subscribers.':'Sends to all students with active subscriptions.'}
            </p>
          </div>
...
export default function AdminPage() {
  ...
  useEffect(()=>{
    ...
    ]).then(([d,r])=>{setStats(d.data.data);setRevenue(r.data.data)}).catch(()=>{})
  },[token,tab])
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',background:'var(--bg-0)',fontFamily:'var(--font-sans)'}}>
      ...
      {/* ── Sidebar ── */}
      <aside style={{width:sidebarOpen?'220px':'60px',flexShrink:0,background:'var(--bg-1)',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',height:'100vh',position:'sticky',top:0,overflow:'hidden',transition:'width 0.25s cubic-bezier(.4,0,.2,1)'}}>
        {/* Logo */}
        <div style={{padding:'1.375rem 1rem 1rem',borderBottom:'1px solid var(--border)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between',gap:'0.5rem'}}>
          ...
          <button onClick={()=>setSidebarOpen(!sidebarOpen)} style={{background:'none',border:'none',color:'var(--txt-3)',cursor:'pointer',display:'flex',padding:'0.25rem',borderRadius:'4px',flexShrink:0}}>
            {sidebarOpen?<ChevronRight size={14}/>:<ChevronDown size={14} style={{transform:'rotate(-90deg)'}}/>}
          </button>
        </div>
        ...
        <nav style={{flex:1,overflowY:'auto',padding:'0.5rem 0.375rem'}}>
          {navItems.map(([id,icon,label])=>(
            <button key={id} onClick={()=>{setTab(id);setImpersonating(null)}}
              ...
              style={{width:'100%',display:'flex',alignItems:'center',gap:'0.5rem',padding:sidebarOpen?'0.625rem 0.75rem':'0.625rem',justifyContent:sidebarOpen?'flex-start':'center',borderRadius:'var(--radius-sm)',background:tab===id?'var(--gold-dim)':'transparent',color:tab===id?'var(--gold)':'var(--txt-2)',fontFamily:'var(--font-sans)',fontSize:'0.8125rem',fontWeight:tab===id?600:400,border:tab===id?'1px solid rgba(201,162,75,0.2)':'1px solid transparent',cursor:'pointer',transition:'all 0.15s',textAlign:'left',marginBottom:'0.1rem',whiteSpace:'nowrap',overflow:'hidden'}}
        ...
        <div style={{padding:'0.5rem 0.375rem',borderTop:'1px solid var(--border)',flexShrink:0}}>
          <a href="/" target="_blank" style={{width:'100%',display:'flex',alignItems:'center',gap:'0.5rem',padding:sidebarOpen?'0.5rem 0.75rem':'0.5rem',justifyContent:sidebarOpen?'flex-start':'center',borderRadius:'var(--radius-sm)',color:'var(--txt-3)',fontSize:'0.78rem',textDecoration:'none',transition:'all 0.15s',whiteSpace:'nowrap',overflow:'hidden',marginBottom:'0.1rem'}}
            onMouseEnter={e=>e.currentTarget.style.color='var(--txt-1)'}
            onMouseLeave={e=>e.currentTarget.style.color='var(--txt-3)'}>
            <Eye size={14} style={{flexShrink:0}}/>{sidebarOpen&&'View Site'}
          </a>
          <button onClick={logout} style={{width:'100%',display:'flex',alignItems:'center',gap:'0.5rem',padding:sidebarOpen?'0.5rem 0.75rem':'0.5rem',justifyContent:sidebarOpen?'flex-start':'center',borderRadius:'var(--radius-sm)',border:'none',background:'transparent',color:'var(--txt-3)',fontSize:'0.78rem',cursor:'pointer',transition:'all 0.15s',whiteSpace:'nowrap',overflow:'hidden'}}
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
        ...
        {/* Mobile header bar */}
        <div className="admin-mobile-header" style={{display:'none',alignItems:'center',justifyContent:'space-between',padding:'0.875rem 1.25rem',background:'var(--bg-1)',borderBottom:'1px solid var(--border)',flexShrink:0,position:'sticky',top:0,zIndex:10}}>
          <div style={{display:'flex',alignItems:'center',gap:'0.625rem'}}>
        ...
        <aside style={{position:'fixed',top:0,left:mobileSidebarOpen?0:'-260px',width:'240px',height:'100vh',background:'var(--bg-1)',borderRight:'1px solid var(--border)',zIndex:80,display:'flex',flexDirection:'column',transition:'left 0.3s cubic-bezier(.4,0,.2,1)',overflowY:'auto'}} className="admin-mobile-nav">
        ...
        {impersonating&&(
          <div style={{padding:'0.625rem 1.5rem',background:'linear-gradient(90deg,rgba(201,162,75,0.12),rgba(201,162,75,0.06))',borderBottom:'1px solid rgba(201,162,75,0.25)',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',flexShrink:0,flexWrap:'wrap'}}>
            <span style={{fontWeight:600,color:'var(--gold)',fontSize:'0.85rem'}}>👀 Viewing as: {impersonating.full_name}</span>
            <button onClick={()=>setImpersonating(null)} style={{display:'flex',alignItems:'center',gap:'0.375rem',padding:'0.375rem 0.75rem',borderRadius:'var(--radius-sm)',border:'1px solid rgba(201,162,75,0.35)',background:'rgba(201,162,75,0.08)',color:'var(--gold)',fontSize:'0.78rem',fontWeight:600,cursor:'pointer'}}>
              ← Exit
            </button>
          </div>
        )}
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'1.5rem',animation:'fadeUp 0.4s ease both'}}>
                    ...
                    {revenue&&(
                      <div style={{padding:'1.75rem',background:'linear-gradient(135deg,var(--bg-3),var(--bg-4))',border:'1px solid rgba(201,162,75,0.2)',borderRadius:'var(--radius-xl)',position:'relative',overflow:'hidden'}}>
                        <div style={{position:'absolute',top:'-20px',right:'-20px',width:'120px',height:'120px',borderRadius:'50%',background:'rgba(201,162,75,0.06)',pointerEvents:'none'}}/>
                        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'1.25rem',flexWrap:'wrap',gap:'1rem'}}>
                          <div>
                            <p style={{fontSize:'0.65rem',fontWeight:700,letterSpacing:'0.18em',textTransform:'uppercase',color:'var(--gold)',marginBottom:'0.375rem'}}>Total Revenue</p>
                            <p style={{fontFamily:'var(--font-serif)',fontSize:'clamp(1.75rem,3vw,2.5rem)',fontWeight:700,color:'var(--txt-1)',lineHeight:1}}>{naira(revenue.total_kobo||0)}</p>
                            <p style={{fontSize:'0.78rem',color:'var(--txt-3)',marginTop:'0.25rem'}}>{revenue.successful_payments} successful payment{revenue.successful_payments!==1?'s':''}</p>
                          </div>
                          <div style={{padding:'1rem 1.25rem',background:'rgba(201,162,75,0.08)',border:'1px solid rgba(201,162,75,0.15)',borderRadius:'var(--radius-md)',textAlign:'right'}}>
                            <p style={{fontSize:'0.6rem',fontWeight:700,letterSpacing:'0.15em',textTransform:'uppercase',color:'var(--gold)',marginBottom:'0.25rem'}}>Last 30 Days</p>
                            <p style={{fontFamily:'var(--font-serif)',fontSize:'1.25rem',fontWeight:700,color:'var(--txt-1)'}}>{naira(revenue.last_30_days_kobo||0)}</p>
                            <p style={{fontSize:'0.7rem',color:'var(--txt-3)',marginTop:'0.15rem'}}>{revenue.last_30_days_count} payment{revenue.last_30_days_count!==1?'s':''}</p>
                          </div>
                        </div>
                        <div style={{display:'flex',gap:'0.875rem',flexWrap:'wrap'}}>
                          {[['Monthly (₦10k)',revenue.monthly_count],['3-Month (₦25k)',revenue.quarterly_count]].map(([l,c])=>(
                            <div key={String(l)} style={{flex:1,minWidth:'120px',padding:'0.75rem',background:'rgba(201,162,75,0.05)',border:'1px solid rgba(201,162,75,0.12)',borderRadius:'var(--radius-md)'}}>
                              <p style={{fontSize:'0.68rem',color:'var(--txt-3)',marginBottom:'0.2rem'}}>{l}</p>
                              <p style={{fontWeight:700,fontSize:'1rem',color:'var(--txt-1)'}}>{c} subscriber{Number(c)!==1?'s':''}</p>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      <style>{`
        ...
        .admin-mobile-nav{display:flex !important;flex-direction:column}
        ...
        {items.map(t=>(
          <div key={t.id} style={{padding:'1.125rem',background:'var(--bg-1)',border:`1px solid ${t.approved?'rgba(201,162,75,0.2)':'var(--border)'}`,borderRadius:'var(--radius-md)',display:'flex',gap:'1rem',alignItems:'flex-start'}}>
            ...
            <div style={{display:'flex',gap:'0.25rem',flexShrink:0}}>
              {!t.approved&&<button onClick={()=>approve(t.id)} style={{width:'30px',height:'30px',borderRadius:'var(--radius-sm)',border:'1px solid rgba(80,200,128,0.3)',background:'rgba(80,200,128,0.08)',color:'#50c880',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><Check size={12}/></button>}
