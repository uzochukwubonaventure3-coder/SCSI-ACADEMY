    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:1.5rem;">
      ${[
        ...
        .map(
          ([k, v]) => `
        <tr>
          <td style="padding:0.625rem;font-size:0.72rem;letter-spacing:0.1em;text-transform:uppercase;color:${brandColors.gold};width:100px">${k}</td>
          <td style="padding:0.625rem;color:${brandColors.text}">${v}</td>
        </tr>
      `
        )
        .join('')}
...
export async function sendRefineryNotification(data: {
  fullName: string
  ...
  const html = baseTemplate(
    'New Refinery Application 🔥',
    `
    <table style="width:100%;border-collapse:collapse;margin-bottom:1.5rem;">
      ${[
        ['Name', data.fullName],
        ['Level', data.levelOrProfession],
        ...
        .map(
          ([k, v]) => `
        <tr>
          <td style="padding:0.5rem;font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:${brandColors.gold};width:100px">${k}</td>
          <td style="padding:0.5rem;color:${brandColors.text}">${v}</td>
        </tr>
      `
        )
        .join('')}
...
export async function sendBroadcastEmail(to: string[], subject: string, bodyHtml: string) {
  if (!resend) {
    console.log(`[EMAIL SKIP] No RESEND_API_KEY. Broadcast to ${to.length} users`)
    return
  }
  if (!to.length) return
