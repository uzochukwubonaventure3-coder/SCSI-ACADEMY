...
import {
  ...
  CreditCard, HelpCircle, Activity, Search,
  Send, BookOpen, UserCheck, Plus, Edit3,
  LayoutDashboard, FileText, Video, Users, Mail,
  MessageSquare, ChevronDown, ChevronRight as ChevronRightIcon,
  ...
  return (
    <Panel title="Phone / WhatsApp" onBack={onBack}>
      <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
        ...
        {canEdit && (
          <button onClick={save} disabled={saving} className="btn btn-gold" style={{ justifyContent:'center', opacity:saving?.7:1 }}>
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      setBio(user.bio || '')
      setAvatar(user.avatarUrl || localStorage.getItem('scsi_avatar') || '')
    }
  ...
  const handleAvatar = (e:React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async ev => {
      ...
      setAvatar(url); localStorage.setItem('scsi_avatar', url); updateLocalUser({ avatarUrl:url })
      try { await axios.put(`${API}/api/paywall/profile`, { fullName:name, bio, avatarUrl:url }, { headers:authH() }); ok('Photo updated!') }
      catch { ok('Photo saved locally') }
    }
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      updateLocalUser({ fullName:name, bio, avatarUrl:avatar }); ok('Profile saved!')
    } catch { err('Save failed. Please try again.') }
    setSaving(false)
  }
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      setPw({ current:'', next:'', confirm:'' }); ok('Password changed!')
    } catch(e:unknown) {
      const er = e as {response?:{data?:{message?:string}}}
          ...
          <SettingsGroup>
            ...
            <div style={{ padding:'0.875rem 1rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                ...
                <p style={{ fontSize:'0.9375rem', fontWeight:600, color:'var(--txt-1)' }}>{user.plan === 'quarterly' ? '3-Month Bundle' : 'Monthly Access'}</p>
              </div>
      ...
      {section === 'profile' && (
        <Panel title="Edit Profile" onBack={() => setSection(null)}>
          <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
            ...
            <button onClick={saveProfile} disabled={saving} className="btn btn-gold" style={{ justifyContent:'center', opacity:saving?.7:1 }}>
      ...
      {section === 'password' && (
        <Panel title="Change Password" onBack={() => setSection(null)}>
          <div style={{ display:'flex', flexDirection:'column', gap:'1.125rem' }}>
            ...
            <button onClick={changePassword} disabled={pwBusy} className="btn btn-gold" style={{ justifyContent:'center', opacity:pwBusy?.7:1 }}>
        ...
        <Panel title="Subscription" onBack={() => setSection(null)}>
          <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
            <div style={{ padding:'1.5rem', background:'linear-gradient(135deg,var(--bg-3),var(--bg-4))', border:'1px solid rgba(201,162,75,0.2)', borderRadius:'16px' }}>
              ...
              <p style={{ fontFamily:'var(--font-serif)', fontSize:'1.375rem', fontWeight:700, color:'var(--txt-1)', marginBottom:'0.25rem' }}>{user.plan === 'quarterly' ? '3-Month Bundle' : 'Monthly Access'}</p>
              {user.expiresAt && (
            ...
            <SettingsGroup>
              <div style={{ padding:'1rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <div><p style={{ fontSize:'0.875rem', fontWeight:600, color:'var(--txt-1)' }}>Monthly Access</p><p style={{ fontSize:'0.78rem', color:'var(--txt-3)' }}>30 days · Renews manually</p></div>
                <span style={{ fontFamily:'var(--font-serif)', fontSize:'1rem', fontWeight:700, color:'var(--gold)' }}>₦10,000</span>
              </div>
              <div style={{ padding:'1rem', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <div><p style={{ fontSize:'0.875rem', fontWeight:600, color:'var(--txt-1)' }}>3-Month Bundle</p><p style={{ fontSize:'0.78rem', color:'var(--txt-3)' }}>90 days · Save ₦5,000</p></div>
                <span style={{ fontFamily:'var(--font-serif)', fontSize:'1rem', fontWeight:700, color:'var(--gold)' }}>₦25,000</span>
              </div>
            </SettingsGroup>
